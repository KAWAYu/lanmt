...
from bilingual_reordering_dataset import BilingualReorderingDataset
from nmtlab.dataset.base import Dataset
from nmtlab.dataset.fixed_iterator import FixedBucketIterator


class MTReorderingDataset(Dataset):
    """Bilingual dataset.
    """

    def __init__(self, corpus_path=None, src_corpus=None, tgt_corpus=None, label_corpus=None,
                 src_vocab=None, tgt_vocab=None,
                 batch_size=64, batch_type="sentence", max_length=60, n_valid_samples=1000, truncate=None):
        ...
        self._label_field = torchtext.data.Field(pad_token="<null>",
                                                 preprocessing=lambda seq: ["<s>"] + seq + ["</s>"])
        self._label_vocab = Vocab()
        self._label_vocab.build(label_corpus)
        self._label_field.vocab = self._label_vocab
        ...
        if corpus_path is not None:
            ...
            # self._data = BilingualDataset(src_corpus, tgt_corpus, self._src_field, self._tgt_field,
            #                               filter_pred=self._len_filter)
            # TODO: add extended BilingualDataset for reordering label
            self._data = BilingualReorderingDataset(src_corpus, tgt_corpus, label_corpus,
                                                    self._src_field, self._tgt_field, self._label_field,
                                                    filter_pred=self._len_filter)
        ...
        valid_data = torchtext.data.Dataset(
            valid_examples,
            fields=[('src', self._src_field), ('tgt', self._tgt_field), ('label', self._label_field)],
            filter_pred=self._len_filter
        )
        ...
        super(MTReorderingDataset, self).__init__(train_data=train_data, valid_data=valid_data, batch_size=batch_size,
                                                  batch_type=batch_type)

    def use_valid_corpus(self, corpus_path=None, src_corpus=None, tgt_corpus=None, label_corpus=None):
        if corpus_path is not None:
            ...
            data = BilingualReorderingDataset(src_corpus, tgt_corpus, label_corpus,
                                              self._src_field, self._tgt_field, self._label_field,
                                              filter_pred=self._len_filter)

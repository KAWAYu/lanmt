    ...
    if OPTS.test or OPTS.all:
        ...
        with open(OPTS.result_path, "w") as outf:
            for i, (line, ordl) in enumerate(zip(lines, orders)):
                ...
                ord_dict = {x: i for i, x in enumerate(ordl)}
                ord_idx = []
                ...
                with torch.no_grad():
                    ...
                    targets, _, prior_states = nmt.translate(x, order=ordl, order_idx=ord_idx)
                    ...
                    for infer_step in range(OPTS.Trefine_steps):
                        ...
                        new_latent, _ = nmt.compute_Q(x, targets, order=ordl, order_idx=ord_idx)
                        targets, _, _ = nmt.translate(x, order=ordl, order_idx=ord_idx,
                                                      latent=new_latent, prior_states=prior_states,
